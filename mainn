local Library = loadstring(game:HttpGet("https://raw.githubusercontent.com/80dropz/Dropzware/main/main.lua"))()

local Window = Library:Create("DropzWare", "Da hood")

local Tab1 = Window:Tab("Aimlocks", true)

Tab1:Label("Aimlock")

Tab1:Button("dropz's Aimlock",function(bool)
...
local Tab2 = Window:Tab("Anti-Lock",false)
 
Tab2:Label("Anti-Lock")
 
Tab2:Button("FIX ANTILOCK",function(bool)
...
end)local Library = loadstring(game:HttpGet("https://raw.githubusercontent.com/80dropz/Dropzware/main/main.lua"))()
...
Tab2:Textbox("AntiLock (use -0.10 to -0.60)","Speed",function(a)
...
local Tab4 = Window:Tab("Visuals", false)
Tab4:Label("Visuals")

Tab4:Keybind("Menu",Enum.KeyCode.V,function()
    Library:Toggle()
end)

-- Variables for ESP and Player Names
...
Tab4:Toggle("ESP Enabled", function(bool)
    ESPEnabled = bool
...
-- Function to create a label for a player's name
function createPlayerNameLabel(player)
    if player.Character then
        local nameLabel = Drawing.new("Text")
        nameLabel.Text = player.Name
        nameLabel.Color = Color3.fromRGB(255, 255, 255) -- White color
        nameLabel.Size = 18 -- Adjust size as needed
        nameLabel.Outline = true
        nameLabel.OutlineColor = Color3.fromRGB(0, 0, 0) -- Black outline for readability
        nameLabel.Center = true -- Center the text
        nameLabel.Visible = true

        playerNameLabels[player.Name] = nameLabel

        -- Update label position every frame
        connections[player.Name] = game:GetService("RunService").RenderStepped:Connect(function()
            if player and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
                local character = player.Character
                local camera = workspace.CurrentCamera
                local head = character:FindFirstChild("Head")
                if head then
                    local screenPos, onScreen = camera:WorldToViewportPoint(head.Position + Vector3.new(0, 3, 0)) -- Adjust height above head
...
-- Function to remove a player's name label
function removePlayerNameLabel(player)
    if playerNameLabels[player.Name] then
        playerNameLabels[player.Name]:Remove()
        playerNameLabels[player.Name] = nil
    end

    -- Disconnect the RenderStepped connection for this player
...
-- Function to toggle showing a player's name
Tab4:Toggle("Show Player Names", function(bool)
    for _, player in pairs(game:GetService("Players"):GetPlayers()) do
        if player ~= game.Players.LocalPlayer then
            if bool then
                createPlayerNameLabel(player)
            else
                removePlayerNameLabel(player)
            end
        end
    end
end)

-- Monitor new players joining and update ESP and name labels
game:GetService("Players").PlayerAdded:Connect(function(player)
    player.CharacterAdded:Connect(function(character)
        if ESPEnabled then
            createESP(player)
        end
        if Tab4:Get("Show Player Names") then
            createPlayerNameLabel(player)
        end
    end)
end)
...
Tab5:Button("RPG",function(value)
    getgenv().game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(113, -26, -267, 5.96046448e-08, 2.91038305e-11, 1, -0.000610388815, 0.999999821, 2.91038305e-11, -0.999999762, -0.000610388815, 5.96046448e-08)
end)
...
tab5:Button("grenade",function(value)
    getgenv().game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(108, -26, -267, -0.0060598203, -1.03402984e-08, -0.999981582, -1.61653102e-09, 1, -1.03306892e-08, 0.999981582, 1.55389912e-09, -0.0060598203)
end)
